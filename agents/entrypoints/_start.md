...
## Before You Begin
1) Read `rules.md` for workspace mission, one-game-at-a-time scope, shared-touchpoint boundaries, and the harness-maintenance rule.
2) Read `agents/outline.md` for repo context.
3) Read `agents/work/task.md` in full. Do NOT open `agents/work/tasksbacklog.md`.
4) Ensure a prompt artifact exists for this task:
...
5) Use the prompt artifact as the authoritative plan for execution.
...
As the final step before writing the status marker, switch to role `historian` (`agents/roles/historian.md`) and prepend a new Builder entry to the very top of `agents/orchestrate_historylog.md`.
...
- Keep game work scoped to one game path at a time unless the task explicitly targets shared infrastructure permitted by `rules.md`.
- If the task changes a published game's player-visible build or release framing, update that game's public version metadata according to `rules.md`.
