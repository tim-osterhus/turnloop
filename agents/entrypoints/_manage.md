...
## Critical Rules
- Read the repo-root `rules.md` (`/mnt/f/_evolve/turnloop/rules.md`) before decomposing the spec. It is the authoritative source for one-game-at-a-time scope, slug/path rules, shared-touchpoint boundaries, and when harness work is actually allowed.
- Process exactly one file per run: the oldest eligible file in `agents/ideas/staging/`.
...
- Placeholder files such as `.gitkeep` are not eligible specs and must never be treated as the selected staging file.

## Inputs (read in order)
1) `README.md`
2) `agents/outline.md`
3) `agents/ideas/staging/` (identify the oldest eligible spec for this run)
4) `agents/work/taskspending.md`
5) `agents/work/tasksbacklog.md`
6) `agents/work/tasksarchive.md`
...
- For game work, keep each card scoped to one game slug at a time plus only the shared touchpoints allowed by `rules.md`.
- Only create harness-maintenance cards when the staged spec explicitly targets maintenance or reliability work that `rules.md` allows.
...
- Avoid duplicates by checking `agents/work/taskspending.md`, `agents/work/tasksbacklog.md`, and `agents/work/tasksarchive.md`.
- Insert cards into `agents/work/taskspending.md` in dependency order.
- If there is no strict dependency order, and tasks currently exist in `agents/work/taskspending.md`, append the new task cards there; do NOT overwrite them.
...
As the final step before writing the status marker, switch to role `historian` (`agents/roles/historian.md`) and prepend a new Manager entry to the very top of `agents/research_historylog.md`.
